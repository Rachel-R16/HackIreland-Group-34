...
def filter_courses(course_estimates, academic_score, relevant_courses, budget_range_for_yearly_fees, tests_taken, scholarship_interest):
    ...
    for university, course in relevant_courses:
        ...
        if not (budget_range_for_yearly_fees["min"] <= tuition_fee <= budget_range_for_yearly_fees["max"]):
            continue

        if tests_taken:
            if not (
                ("SAT" in tests_taken and int(tests_taken["SAT"]) >= int(sat_score)) or
                ("IELTS" in tests_taken and int(tests_taken["IELTS"]) >= int(ielts_score)) or
                ("ACT" in tests_taken and int(tests_taken["ACT"]) >= int(act_score)) or
                ("TOEFL" in tests_taken and int(tests_taken["TOEFL"]) >= int(toefl_score))
            ):
                continue
